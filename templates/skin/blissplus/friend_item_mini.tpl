...
{* if $oUserFriend and ($oUserFriend->getFriendStatus()==$USER_FRIEND_ACCEPT+$USER_FRIEND_OFFER or $oUserFriend->getFriendStatus()==$USER_FRIEND_ACCEPT+$USER_FRIEND_ACCEPT)}
    <li id="delete_friend_item"><a href="#" class="button button-actions friendadd" title="{$aLang.user_friend_del}" onclick="return ls.user.removeFriend(this,{$oUserProfile->getId()},'del');"><i class="icon-bliss-mini-minus"></a></li>
{elseif $oUserFriend and $oUserFriend->getStatusTo()==$USER_FRIEND_REJECT and $oUserFriend->getStatusFrom()==$USER_FRIEND_OFFER and $oUserFriend->getUserTo()==$oUserCurrent->getId()}
    <li id="add_friend_item"><a href="#"  title="{$aLang.user_friend_add}" onclick="return ls.user.addFriend(this,{$oUserProfile->getId()},'accept');"><i class="icon-bliss-mini-plus"></i></a></li>
...
{elseif !$oUserFriend}
    <div id="add_friend_form" class="modal">
        ...
        <form onsubmit="return ls.user.addFriend(this,{$oUserProfile->getId()},'add');" class="modal-content">
            ...
            <button type="submit" class="button button-primary friendadd">{$aLang.user_friend_add_submit}</button>
        </form>
    </div>
    <li id="add_friend_item"><a href="#" class="button button-actions friendadd" title="{$aLang.user_friend_add}" id="add_friend_show"><i class="icon-bliss-mini-plus"></i></a></li>
{else}
    <li id="add_friend_item"><a href="#" class="button button-actions friendadd" title="{$aLang.user_friend_add}" onclick="return ls.user.addFriend(this,{$oUserProfile->getId()},'link');"><i class="icon-bliss-mini-plus"></i></a></li>
{/if *}
