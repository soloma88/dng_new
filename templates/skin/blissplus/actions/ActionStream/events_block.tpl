	...
	{foreach from=$aStreamEvents item=oStreamEvent}		
		...
		<li class="stream-item-type-{$oStreamEvent->getEventType()}">
            ...
            {elseif $oStreamEvent->getEventType() == 'add_comment'}
                ...
                {* if trim($sTextEvent)}
                    <div class="block-stream-comment-preview">{$sTextEvent}</div>
                {/if*}
            ...
            <p class="date" title="{date_format date=$oStreamEvent->getDateAdded()}">{date_format date=$oStreamEvent->getDateAdded() hours_back="12" minutes_back="60" now="60" day="day H:i" format="j F Y, H:i"}</p>
        </li>
