...
<head>
    ...
    {if {cfg name='plugin.blissplus.header_height'} && $oConfig->get('plugin.blissplus.header_bg') }
        <style>
            #nav {
               height:  {cfg name='plugin.blissplus.header_height'}px!important;
            }
        </style>
    {/if}
...
<body class="{$body_classes} width-{cfg name='view.grid.type'}">
	...
	<div id="container" class="{if $noSidebar}no-sidebar{/if} {hook run='container_class'}">
