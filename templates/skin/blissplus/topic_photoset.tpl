...
<div class="topic-photo-preview" id="photoset-main-preview-{$oTopic->getId()}" onclick="window.location='{$oTopic->getUrl()}#photoset'">
	<div class="topic-photo-count" id="photoset-photo-count-{$oTopic->getId()}">{$oTopic->getPhotosetCount()}</div>
	
	{if $oMainPhoto->getDescription()}
		<div class="topic-photo-desc" id="photoset-photo-desc-{$oTopic->getId()}">{$oMainPhoto->getDescription()}</div>
	{/if}

	<img src="{$oMainPhoto->getWebPath(1000)}" alt="image" id="photoset-main-image-{$oTopic->getId()}" />
</div>
{/if}
...
<div class="topic-content text">
    ...
    {if $bTopicList}
        ...
        {if $oTopic->getTextShort()!=$oTopic->getText()}
            <a href="{$oTopic->getUrl()}#cut" style="margin-left: 8px;" title="{$aLang.topic_read_more}">
