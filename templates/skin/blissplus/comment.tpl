...
<section id="comment_id_{$oComment->getId()}" class="comment
	...
	{if !$oComment->getDelete() or $bOneComment or ($oUserCurrent and $oUserCurrent->isAdministrator())}
		...
		<ul class="comment-info">
            ...
            <li class="comment-author" style="width: auto;padding-right: 12px;">
            ...
            {if $oComment->getPid()}
                <li class="goto-comment-parent" style="width: auto;"><a href="#" onclick="ls.comments.goToParentComment({$oComment->getId()},{$oComment->getPid()}); return false;" title="{$aLang.comment_goto_parent}">↑</a></li>
            {/if}
            <li class="goto-comment-child" style="width: auto;"><a href="#" title="{$aLang.comment_goto_child}">↓</a></li>
			...
			{if $oComment->getTargetType() != 'talk'}						
				<li id="vote_area_comment_{$oComment->getId()}" class="vote 
																		...
																		{if $oVote} 
																			...
																			{/if}
																		{/if}">
                    <div class="vote-down" onclick="return ls.vote.vote({$oComment->getId()},this,-1,'comment');"><i class="fa fa-minus-square"></i></div>
                    <span class="vote-count" id="vote_total_comment_{$oComment->getId()}">{if $oComment->getRating() > 0}+{/if}{$oComment->getRating()}</span>
					<div class="vote-up" onclick="return ls.vote.vote({$oComment->getId()},this,1,'comment');"><i class="fa fa-plus-square"></i></div>
